...
This file is a high-level checklist only. For the strict current audit, see
...
|---|---|---|
| I. Regles absolues | partially covered | provenance files, protocol deviation log, environment capture, no secret persistence in article3 artifacts, but several prompt-grade freeze and invalidation rules remain incomplete |
...
| VI. Etude bibliographique | partially covered | `literature/` populated and expanded with additional primary sources on FrugalGPT, RouteLLM, Hybrid LLM, conformal routing, InferenceDynamics, FLARE, MTRouter, RouteLMT, and Libra, but final saturation pass and prompt-grade matrix schema remain pending |
...
| X. Tests logiciels obligatoires | partially covered | targeted Go and Helm validations pass on the GOV-AR surface, but the full prompt matrix of race, lint, kubeconform, image, and end-to-end restart/failure tests is still incomplete |
...
| Final article, PDF, Overleaf, replication bundle | partially covered | manuscript hardened and recompiled, PDF produced, Overleaf zip produced, replication bundle zip produced, and prompt-named final artifact reports materialized; however the manuscript is not yet Q1 submission-ready and multiple release gates remain open |
...
- full prompt-scale experiments and statistical analysis
- figures, tables, and claims-to-evidence completion
