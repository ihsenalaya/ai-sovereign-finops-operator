#!/usr/bin/env python3
"""Reject mutable or unevidenced external FROM references in operator images."""
...
DOCKERFILES = sorted((ROOT / "operateur").glob("Dockerfile*"))
...
def validate(dockerfiles: list[Path], provenance: Path) -> list[str]:
    ...
    for dockerfile in dockerfiles:
        for number, raw_line in enumerate(
            ...
            if not line.upper().startswith("FROM "):
                continue
            fields = line.split()
            location = f"{dockerfile}:{number}"
            try:
                image = fields[1] if not fields[1].startswith("--platform=") else fields[2]
            except IndexError:
                errors.append(f"{location}: malformed FROM instruction")
                continue
            if image == "scratch":
                continue
            ...
            if not match:
                errors.append(
                    f"{location}: external FROM must retain a readable tag and pin a "
            ...
            if evidence is None:
                errors.append(f"{location}: pin is absent from {provenance}")
                continue
            if evidence.get("index_digest") != match.group("digest"):
                errors.append(f"{location}: provenance index digest does not match FROM")
            if evidence.get("platform") != "linux/amd64":
                errors.append(f"{location}: linux/amd64 manifest verification is absent")
            if not re.fullmatch(
                r"sha256:[0-9a-f]{64}", evidence.get("platform_manifest_digest", "")
            ):
                errors.append(f"{location}: invalid linux/amd64 platform digest")
...
def main() -> int:
    ...
    parser.add_argument("--dockerfile", type=Path, action="append")
    parser.add_argument("--provenance", type=Path, default=PROVENANCE)
    args = parser.parse_args()
    dockerfiles = args.dockerfile or DOCKERFILES
    errors = validate(dockerfiles, args.provenance)
    if errors:
        for error in errors:
