...
The live \texttt{AIFinOpsReport} for \texttt{ai-report-all} reached a total estimated cost of 0.022547 EUR. The dominant cost contributor was \texttt{finance/risk-assistant} on \texttt{gpt-us-mini}, with 0.016477 EUR attributed to a non-compliant US provider. To move E1 beyond a controller smoke test, we independently recomputed the cost attribution from the raw Envoy AI Gateway token telemetry in \texttt{metrics/gateway\_metrics.prom} and the declared provider prices. That ground-truth reconstruction yielded 0.02254667 EUR total cost versus 0.022547 EUR in the report, with mean per-triplet absolute error \(2.68\times10^{-7}\) EUR and mean absolute percentage error 0.080\%, the latter arising only from micro-EUR rounding in the serialized report values. The top-model ranking was reproduced exactly (\texttt{gpt-us-mini} first, then \texttt{mistral-large-latest}, then \texttt{gpt-france-mini}).
