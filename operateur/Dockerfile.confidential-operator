# Build the standalone confidential-computing / attestation controller manager
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -trimpath -buildvcs=false -ldflags='-s -w -buildid=' -o manager ./cmd/confidential-manager/main.go
