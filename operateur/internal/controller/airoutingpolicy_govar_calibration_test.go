...
func calibrationFixture(t *testing.T, now time.Time) (*AIRoutingPolicyReconciler, *aiopsv1alpha1.AIRoutingPolicy, *corev1.ConfigMap, *corev1.ConfigMap) {
	...
	return &AIRoutingPolicyReconciler{Client: client, Scheme: scheme, Now: func() time.Time { return now }}, policy, calCM, monitorCM
}
...
func TestCalibrationProducerValidStaleDriftAndRevalidation(t *testing.T) {
	...
	if status == nil || status.Calibration == nil || !status.Calibration.Valid || status.Drift.ConservativeMode || status.Calibration.AdaptiveOutputTokens != 400 {
