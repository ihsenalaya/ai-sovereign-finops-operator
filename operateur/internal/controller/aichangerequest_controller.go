...
func (r *AIChangeRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if crq.Status.ExpiresAt == nil || !crq.Status.ExpiresAt.Time.Equal(expiresAt) {
...
func (r *AIChangeRequestReconciler) approveGOVARRoute(ctx context.Context, crq *aiopsv1alpha1.AIChangeRequest) (ctrl.Result, error) {
	...
	if crq.Status.Phase == aiopsv1alpha1.AIChangeRequestPhaseApproved && crq.Status.ApprovedAt != nil && crq.Status.ObservedGeneration == crq.Generation && crq.Status.ApprovedScopeDigest == scope.ScopeDigest {
	...
	remaining := scope.ValidUntil.Time.Sub(r.now())
