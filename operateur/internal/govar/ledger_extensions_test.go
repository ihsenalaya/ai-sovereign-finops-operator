...
func TestBudgetWindowRolloverGuardAndFinality(t *testing.T) {
	...
	a, err := e.Admit(admitRequest("old", testTenant, testWorkload), b, defaultRouting(), defaultCandidates())
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = e.Admit(admitRequest("new", testTenant, testWorkload), b, defaultRouting(), defaultCandidates()); err != nil {
		t.Fatal(err)
	}
...
func TestLateSettlementDownCorrectionNoCreditAndDebtPayoff(t *testing.T) {
	...
	b.Spec.Period = "daily"
	b.Spec.BudgetEUR = resource.MustParse("0.02")
	a, err := e.Admit(admitRequest("late", testTenant, testWorkload), b, defaultRouting(), defaultCandidates())
	if err != nil {
		t.Fatal(err)
	}
	_, _, _ = e.Dispatch(dispatchRequest("late", "claim", a.ProviderAttemptID, DispatchClaimed, testTenant, testWorkload))
	now = now.Add(24 * time.Hour)
	if _, err = e.Admit(admitRequest("new2", testTenant, testWorkload), b, defaultRouting(), defaultCandidates()); err != nil {
		t.Fatal(err)
	}
...
func TestProviderAttemptBindingAndExpiryReconciliation(t *testing.T) {
	...
	e.now = func() time.Time { return now }
	a := mustAdmit(t, e, "bound", testTenant, testWorkload, defaultCandidates())
