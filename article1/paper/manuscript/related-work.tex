...
\begin{table}[t]
  \centering
  \caption{Positioning vs.\ the closest confidential-Kubernetes systems.
  \emph{Sched.} = attestation is consulted at scheduling time; \emph{TOCTOU} =
  removes the externally schedulable check-to-bind window; \emph{Verif.\
  artifact} = emits an offline-verifiable placement proof; \emph{Freshness} =
  enforces evidence age/revocation at bind.}
  \label{tab:related}
  \footnotesize
  \begin{tabular}{@{}lccccc@{}}
    ...
    \midrule
    CoCo/Trustee~\cite{coco2024} & pod & --- & --- & --- & runtime \\
    dstack~\cite{yang2026dstack} & pod & --- & --- & --- & runtime \\
    Constellation & cluster & --- & --- & --- & boot \\
    C8s (conf.\ control plane) & cluster & --- & --- & --- & --- \\
    \texttt{schedulingGate}+ctrl.\ (B4) & node & \checkmark & partial & --- & \checkmark \\
    \textbf{This work (B5)} & node & \checkmark & \checkmark & \checkmark & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}
