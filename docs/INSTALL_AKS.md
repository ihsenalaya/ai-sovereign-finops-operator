...
# Avec ACR :
az acr login -n <acr>
docker build -t <acr>.azurecr.io/ai-sovereign-finops-operator:0.1.0 .
docker push <acr>.azurecr.io/ai-sovereign-finops-operator:0.1.0
```
...
Adapter `config/samples` à votre contexte (provider `azure-openai`, `dataResidency: france`,
pricing réel) puis `kubectl apply -k config/samples/`.

## 5. Observabilité
Activer le `ServiceMonitor` si Prometheus Operator est présent :
`--set metrics.serviceMonitor.enabled=true`. Importer `dashboards/ai-finops-overview.json` dans Grafana.
